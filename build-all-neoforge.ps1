# build-all-neoforge.ps1 -- ASR unified NeoForge source. (No 26.3 NeoForge yet.)
...
foreach($v in $Versions){
  ...
  Write-Host "=== ASR NeoForge $v (neo=$($m.neo)) ==="
  Push-Location $nf
  ...
  $jar=Get-ChildItem (Join-Path $nf "build\libs") -Filter "autoshield-reborn-*.jar"|?{$_.Name -notmatch 'sources|slim'}|Sort-Object LastWriteTime|Select-Object -Last 1
  Copy-Item $jar.FullName (Join-Path $dist ("autoshield-reborn-{0}+{1}-neoforge.jar" -f $modver,$v)) -Force
  Write-Host "  -> $v done"
}

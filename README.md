...
- [26.2](https://github.com/Kishku7/autoshield-reborn/tree/26.2)
- [26.1.2](https://github.com/Kishku7/autoshield-reborn/tree/26.1.2)
...
block any attack a shield can normally block â€” melee, projectiles, and more â€” with no need to raise
it. Blocking only applies to threats from within a **180Â° frontal arc**: damage from behind still
lands. The durability your shield loses per blocked hit is **server-configurable (0â€“10, default 1)**.
...
configurable durability cost, an in-game config screen, and server-authoritative operator sync â€”
across Fabric and NeoForge on Minecraft 26.x. Credit and thanks to agorasim20 for the original idea.

## Features

- **Auto-block while holding a shield** in the main or off hand â€” no right-click needed.
...
- **180Â° facing gate** â€” only blocks attacks from your forward hemisphere (horizontal head facing).
- **Server-set durability cost (0â€“10, default 1).** 0 means the shield never wears from auto-blocks.
...
- **Dedicated-server safe.** All blocking logic is server-authoritative â€” fully functional with no
...
|-----------|------|---------|
| 26.1.2 (Fabric also 26.1 / 26.1.1) | `autoshield-reborn-1.0.0+26.1.2.jar` â€” **one universal jar** | Fabric **and** NeoForge |
| 26.2 (incl. 26.2-rc-1) | `autoshield-reborn-1.0.0+26.2.jar` | Fabric |

The 26.1.2 download is a single merged jar that runs on **both** Fabric and NeoForge â€” just drop it in
`mods/`. NeoForge for 26.2 will be added once NeoForge ships a 26.2 build. On Fabric 26.2, ModMenu had
not yet published a 26.2-compatible release at time of writing â€” the JSON config and all blocking still
work without it.
...
`durabilityCost` is clamped to 0â€“10 â€” the durability a shield loses each time it auto-blocks a hit.
Change it in-game from the config screen (operators only on a server) or by editing this file.

## Source layout (branches)

- `main` â€” this landing page.
- `26.1.2` â€” source for the 26.1.2 universal release: `fabric/` (fabric-loom) and `neoforge/`
...
- `26.2` â€” `fabric/` source targeting 26.2-rc-1 (JDK 25, fabric-loom).

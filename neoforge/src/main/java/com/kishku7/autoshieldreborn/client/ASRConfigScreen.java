...
/**
 * Auto-Shield Reborn config screen (NeoForge), opened from the mods-list "Config" button. Identical
 * to the Fabric screen except the operator's change is sent with NeoForge's ClientPacketDistributor.
 * Shows the server's durability cost (0-10); operators (and the single-player host) get an editable
 * slider, everyone else sees it read-only with a "server-controlled" note.
 */
    ...
    protected void init() {
        ...
        this.addRenderableWidget(Button.builder(Component.literal("Done"), b -> {
            if (this.editable && this.slider.getValue() != ASRConfig.durabilityCost()) {
                ClientPacketDistributor.sendToServer(new SetDurabilityPayload(this.slider.getValue()));
            }
            this.onClose();
